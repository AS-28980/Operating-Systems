...
bool boundedCAS(std::atomic<int>& val, int& expected, int new_val) 
{
    return val.compare_exchange_weak(expected, new_val);
}
...
void criticalSection() 
{
    ...
    while (true) 
    {
        int current = C.load(memory_order_relaxed);
        if (current >= N) break; 

        int expected = current;
        if (!boundedCAS(C, expected, min(N, current + R)))
            continue;
...
int main() 
{
    ...
    std::ofstream outfile("out_BCAS.txt");
