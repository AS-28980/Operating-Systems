...
{
    ofstream logfile("RW-log.txt", ios::app); // Open the file in append mode
    if (logfile.is_open()) 
    {
...
void calculateAndWriteAverageTimes() 
{
    ...
    ofstream avgFile("Average_Time_rw.txt");
    if (avgFile.is_open()) 
    {
...
void calculateAndWriteWorstTime()
{
    ...
    ofstream avgFile("Worst_Time_rw.txt");
    if (avgFile.is_open()) 
    {
        ...
        cerr << "Error: Unable to open Worst_Time_rw.txt file!" << endl;
}
